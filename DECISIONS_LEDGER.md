...
- **In-app notifications are confirmed working end to end**: `fire()` → invoke → JWT auth → role-based recipient resolution → `notifications` insert → RLS read-back → Notification Center. This is the first delivery-boundary proof in the notification stack.
- **Push send remains unproven.** The HTTP v1 transport has still never been exercised against Google. It is statically checked and its credential validated, nothing more.

**Known gaps, unfixed and deliberate.**
- **No foreground notification.** `FcmService` handles `onMessageOpenedApp` and `getInitialMessage` only — there is no `FirebaseMessaging.onMessage` handler and no local-notifications plugin. Android does not auto-display while the app is foregrounded, so a pop-up appears **only when the app is backgrounded or terminated**. Closing this needs either an in-app banner (no new dependency) or `flutter_local_notifications` + a declared Android notification channel — note `fcm.ts` deliberately sends no `channel_id` because the app declares none.
...
**Process.** Four defects in one day, all the §10 shape. The rule earned another restatement: **a feature is done when its effect is observed, not when its code exists.** In each case the code was correct and the call site, caller, or target was missing — and every failure was silent, because each layer treated "nothing to do" as success.
