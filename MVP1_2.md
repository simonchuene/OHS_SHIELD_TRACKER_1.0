...
**Now proven on-device:** `device_tokens` registers real tokens; **in-app notifications work end to end**; and **push delivery is confirmed** — a `capa.assigned` push raised a system notification on the owner's handset, closing the delivery-boundary item §11 opened. A fifth defect (§12 D-push-4) was found and fixed: the CAPA repo never cached server rows, so an owner acting on a CAPA created on another device cached a one-field fragment and the detail screen threw "Unexpected error". **Foreground notifications** are now surfaced as an in-app banner with a deep-linking View action. **Scheduled sweep added 2026-08-15 (Ledger §13):** `notify-sweep` (Edge Function) + migration **0017** (pg_cron/pg_net, daily 06:00 UTC) raise `capa.overdue` and `inspection.due` — triggers nothing in the app could fire, because they become true with the passage of time. Owner-only recipients, idempotent to one notification per entity per day. Config lives in Supabase Vault (`alter database ... set` is not permitted on hosted Supabase). **Also fixed:** every absolute timestamp rendered N hours behind — Postgres `timestamptz` parses to a UTC `DateTime` and `DateFormat` printed UTC wall-clock, so CAT/SAST users saw −2h across notifications, audit, incidents and reports. **Verified on device 2026-08-16 (Ledger §14):** foreground banner, CAT timestamps, CAPA "Start work", and the incident-trend chart overflow fix — all observed on an Android 15 tablet. The **scheduled sweep ran unattended at 06:00 UTC**, raising a `capa.overdue` notification with nobody involved, with its once-per-day guard holding. Notifications are now proven end to end across in-app, push, scheduled and foreground paths. **UX round 2026-08-16 (Ledger §15):** client-side RBAC affordances (actions a role cannot perform are now disabled with a reason, across all five workflow screens, via a shared `RankGatedAction`); status steppers + Status/Next lines everywhere; selection dialogs now **confirm before committing**; assignee names shown on CAPA and investigation; notification small icon added. Also fixed **D-capa-1** — assigning an owner wrote only `owner_id` and left the CAPA in *Created*, so the assignee met a greyed-out "Assign" and could never reach "Start work". **CAPA completion notes 2026-08-16 (Ledger §17, migration 0018):** the assigned owner can now submit their own work for verification — previously that step was Safety Officer+, so an owner could Start work but not hand it back — and records **what was done** in a new `completion_notes` field, kept separate from the verifier's `verification_notes` so the audit trail shows who wrote which. Surfaced on the CAPA screen and as a *Work completed* column in the CAPA status report. Closing remains Safety Officer+. **Verified end-to-end on device 2026-08-17 (Ledger §15.6):** RBAC gating on a genuinely lower-ranked account, the notification icon, the foreground banner, the Users screen, and the full owner CAPA path — Start work → Submit with completion notes → a different Safety Officer notified → Verify & close still barred to the owner. **Auth email flows fixed 2026-08-17 (Ledger §21, migration 0020):** invite and password reset both sent successfully and their links went nowhere — neither passed `redirectTo`, so Supabase fell back to the default Site URL `http://localhost:3000`; the app also registered no deep link and had no set-password screen. All three built. Invite completion is detected from server state (`status = 'invited'` now holds until a password is set) rather than from an auth event, which an invite shares with an ordinary login. **Pre-release blockers found:** the built-in SMTP allows only ~2–4 emails/hour so production needs custom SMTP; and a custom scheme only opens on a device with the app installed, so desktop-opened invites need an App Link. **Remaining gap:** a true foreground heads-up notification (needs `flutter_local_notifications` and a notification channel — MVP2 polish); incidents have no assignee column, so ownership is tracked via their linked investigation/CAPAs.
