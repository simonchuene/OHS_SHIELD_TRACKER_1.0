...
| Release build (`flutter build appbundle --release --flavor prod`) — **never run on any toolchain** | §19 | Not started. **Found while checking (not yet addressed):** (1) the `release` build type is signed with the **debug key** — Flutter's template default in `android/app/build.gradle.kts` — which Google Play rejects, and no upload keystore exists; (2) **only the `dev` flavor has a `google-services.json`**, so `uat` and `prod` cannot currently build at all — the Firebase Gradle plugin fails without it; (3) no `config/env/prod.json`, only the example. The riskiest part — R8 shrinking breaking reflection-based plugins such as Firebase Messaging — can be tested now with `--release --flavor dev`, independent of all three ___ |
...
| Remove the DEBUG-ONLY corporate-CA trust before any release build | MVP1_2.md | **Done.** The trust was already applied only under `kDebugMode` (`main.dart`), but `pubspec.yaml` listed `assets/dev/corporate_ca.pem` unconditionally, so **the certificate shipped inside every build, release included**. It is now bundled into the `dev` flavor only — uat and prod carry no copy in any build mode — and `DevHttpOverrides.load()` already treats a missing file as "no override". Pinned by `test/core/network/corporate_ca_asset_test.dart`, which checks the asset manifest: verified passing under no flavor, `dev`, `uat` and `prod`, and **failing against the old `pubspec`** (no flavor and `prod`). A `dev` APK was built and inspected: the file is inside it |
